...
class Reports extends Component {
  ...
  render() {
     ...
     if(this.state.transactions){
       ...
       transactions = listTransaction.map((transaction,index) => <Report sequenceNo={index} key={transaction._id}  changeStatus={(transaction) => this.changeStatusHandler(transaction)} transaction={transaction}/>);      
     }
     ...
     const externalCloseBtn = <button className="close" style={{ position: 'absolute', top: '15px', right: '15px' }} onClick={this.toggle}>&times;</button>;    
      ...
      transactionsExcel = dataSet1.map((transaction,index) => <Report sequenceNo={index} key={transaction._id}   transaction={transaction}/>);      
      ...
      <div className="animated fadeIn">
        <Row>
          <Col>
            <Card>
              <CardHeader>
                <i className="fa fa-align-justify"></i> Transaction Report       
              </CardHeader>
              <CardBody>
              <Form noValidate>
                 <Row>
