...
class MyTrades extends React.Component {
  ...
  componentWillMount() {
    axios
      .get("/product/productDetails/" + this.state.productId)
      .then(result => {
        this.setState({
          resultData: result.data.result,
          mainImages: result.data.result
            ? result.data.result.productImages[0]
            : "default_product_img@3x.png",
  ...
  render() {
    ...
    if (this.state.conditions) {
      ...
      conditionsList.map((key, index) => {
        if (key.id == 4) {
          optionTemplate = key.name;
        }
      });
    }
    let img = this.state.resultData.userId
      ? this.state.resultData.userId.profilePic
      : "";
    let description = this.state.resultData.description
      ? this.state.resultData.description
      : "";
    let userid = this.state.resultData.userId
      ? this.state.resultData.userId._id
      : "";
    return (
      <div>
          ...
          <If condition={this.state.isAlreadyPitched}>
            <Then>
              <div className="topMsg">
                You have already pitched on this product
              </div>
            </Then>
          </If>
            ...
            <div className="detail-div">
              <If condition={this.state.resultData.length === 0}>
                ...
                <Else>
                  <div className="pic">
                    <ThumbGallery
                      galleriesID={this.state.productId}
                      galleriesImg={this.state.mainImages}
                    />
                  </div>
                  <div className="details">
                    ...
                    <p className="tagsrow">
                      {this.state.resultData.productCategory
                        ? this.state.resultData.productCategory.title
                        : ""}
                    </p>
                    ...
                    <div className="brdr-top no-padding ">
                      <div className="ratingRow">
                        ...
                        <div className="pic">
                          ...
                          >
                            <img
                              ...
                              src={
                                constant.BASE_IMAGE_URL + "ProfilePic/" + img
                              }
                              alt=""
                            />
                          </Link>
                        ...
                        <p>
                          <Link
                            ...
                            to={"/public-profile/" + userid}
                          >
                            {this.state.resultData.userId
                              ? this.state.resultData.userId.firstName
                              : ""}
                          </Link>
                        </p>
                      ...
                      <If condition={localStorage.getItem("isLoggedIn") == "1"}>
                        <Then>
                          ...
                          >
                            ...
                            <Else>
                              <LastPitchPopup
                                offerTrade={this.state.resultData}
                                proID={this.state.productId}
                              />
                            </Else>
                          </If>
                          <If
                            condition={this.state.isAlreadyInWishlist === false}
                          >
                            <Then>
                              <a
                                href="#"
                                className="ditch add-wishlist"
                            ...
                            <Else>
                              <span className="ditch add-wishlist">
                                Added in Wishlist
                              </span>
                            </Else>
                          </If>
                      ...
                      <table cellPadding="0" cellSpacing="0" width="100%">
                        <tbody>
                          <tr>
                            <td>Size:</td>
                            <td>
                              {this.state.resultData.size
                                ? this.state.resultData.size.size
                                : ""}{" "}
                              GB
                            </td>
                          </tr>
                          <tr>
                            ...
                            <td>
                              <img src={colorOrange} />
                              {this.state.resultData.color}
                            </td>
                          </tr>
                          <tr>
                            <td>Brand:</td>
                            <td>
                              {this.state.resultData.brand
                                ? this.state.resultData.brand.brandName
                                : ""}
                            </td>
                          </tr>
                          <tr>
                          ...
                          <tr>
                            <td>Age:</td>
                            <td>{this.state.resultData.productAge}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
