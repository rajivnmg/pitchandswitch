...
class ShippingTypePopup extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .all([
        ...
        axios.get(
          "trade/getShippingCost/" +
            this.props.offerTrade._id +
            "/" +
            this.props.productID
        )
      ])
      .then(
        axios.spread((ruser, rsShippingCost) => {
          this.setState({
            pitchUserDATA: ruser.data.result,
            shippingCost: rsShippingCost.data.result
          });
        })
      )
  ...
  _renderSuccessMessage() {
    return (
      <div className={"alert alert-success mt-4"} role="alert">
        Congratulation!!! You have successfully Switch{" "}
        <Link to={"/dashboard"}> Go to Dashboard </Link> Or it will automaticaly
  ...
  render() {
    ...
    const productIMG = this.state.pitchUserDATA
      ? this.state.pitchUserDATA.profilePic
      : "";

    return (
      <Popup
        trigger={<a className="TradeInfobtn"> Switch </a>}
        modal
              ...
              <Then>
                <div className="modal pitchSuccessful">
                  ...
                  <div className="header centerheading">
                    <span>Switch</span> Successful
                    <div className="cl" />
                  </div>
                  <p className="textSuccessful">
                    <span classNamne="gray">
                      You have successfully Switch on
                    </span>
                    {this.props.offerTrade.productName} ~{" "}
                    {this.props.offerTrade.description}
                  </p>
                    ...
                    <div className="tab-outer-container new_poup">
                      <Tabs>
                        <TabList className="choose-shippinh-type">
                          <Tab>
                            <img src={truckImg} alt="" /> Use Logistic
                          </Tab>
                          <Tab>
                            <img src={twoType} alt="" /> Meet in Persone
                          </Tab>
                        </TabList>
                        {/* first tab data */}
                        <TabPanel>
                          <div className="return-request-form">
                            ...
                            <div className="form-row">
                              ...
                              <div className="yellow-brdr-box-div ">
                                ...
                                this.state.shippingCost.length > 0
                                  ? this.state.shippingCost.map(
                                      (rate, index) => {
                                        return (
                                          <Auxios key={index}>
                                            <div className="radioBtn">
                                              <input
                                                id={"shippingRate" + index}
                                                required={true}
                                                name={"shippingRate"}
                                                type={"radio"}
                                                onChange={e =>
                                                ...
                                                value={rate.rate}
                                                data-stypeid={rate.id}
                                                data-shipmentid={
                                                  rate.shipment_id
                                                }
                                              />
                                              <label
                                                htmlFor={"shippingRate" + index}
                                              />
                                            </div>
                            ...
                            <div className="form-row">
                              <input
                                type="submit"
                                className=""
                                value={"Pay $" + this.state.paymentAmount}
                              />
                            </div>
                            <div className="form-row no-padding">
                              <p className="secure-img"> Secure payments</p>
                              <p className="secure-btm-text">
                                This payments use a secure 128-bit SSL
                                encryption
                              </p>
                            </div>
                          </div>
                        ...
                        <TabPanel>
                          <div className="return-request-form">
                            <div className="brdr-box-div">
                              ...
                              <div className="user_right">
                                <h2>
                                  {this.props.offerTrade.pitchUserId
                                    ? this.props.offerTrade.pitchUserId.userName
                                    : ""}
                                </h2>
                                <p>
                                  {this.props.offerTrade.pitchUserId
                                    ? this.props.offerTrade.pitchUserId.address
                                    : ""}
                                </p>
                                <p>
                                  {this.state.pitchUserDATA.city
                                    ? this.state.pitchUserDATA.city.cityName
                                    : ""}
                                  ,
                                  {this.state.pitchUserDATA.state
                                    ? this.state.pitchUserDATA.state.stateName
                                    : ""}
                                </p>
                                <p>
                                  {this.state.pitchUserDATA.country
                                    ? this.state.pitchUserDATA.country
                                        .countryName
                                    : ""}
                                </p>
                                <p>
                                  {this.state.pitchUserDATA
                                    ? this.state.pitchUserDATA.zipCode
                                    : ""}
                                </p>
                                <p>
                                  {this.state.pitchUserDATA
                                    ? this.state.pitchUserDATA.phoneNumber
                                    : ""}
                                </p>
                              </div>
                            </div>
                          ...
                          <div className="form-row">
                            <Button
                              onClick={e =>
                                this.submitHandler(this.state.productID)
                              }
                              color="success"
